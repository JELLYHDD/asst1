...
void sqrtAVX2(int N,float startGuess,float* values,float* output)
{
    ...
    __m256 minthresh=_mm256_set1_ps(0.9999f);
    __m256 maxthreash=_mm256_set1_ps(1.0001f);
    __m256 mask1,mask2,mask;
    __m256 guess,tmp,value;
    float cmp[8];
    for(int i=0;i<N;i+=8)
    {
        ...
        mask1=_mm256_cmp_ps(tmp,maxthreash,_CMP_GT_OS);
        mask2=_mm256_cmp_ps(tmp,minthresh,_CMP_LT_OS);
        mask=_mm256_or_ps(mask1,mask2);
        _mm256_storeu_ps(cmp,mask);
        while(cmp[0]||cmp[1]||cmp[2]||cmp[3]||cmp[4]||cmp[5]||cmp[6]||cmp[7])
        {
            tmp=_mm256_sub_ps(three,tmp);
            guess=_mm256_mul_ps(tmp,guess);
            guess=_mm256_mul_ps(half,guess);
